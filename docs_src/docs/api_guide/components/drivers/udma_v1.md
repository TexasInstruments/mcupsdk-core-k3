...
- PDMA module to initiate transfers to/from PDMA peripherals like UART, McASP, McSPI
- DMA transfer to/from from native PSIL peripherals like CPSW
- Event and interrupt management like DMA completion, channel chaining, interrupt sharing, Rx flow, Resources management across instances and cores for DMA channels is handled.
...
-  UDMA driver doesn't manage/allocate the descriptor and ring memory. The caller need to allocate and provide the required memory.
...
\imageStyle{DMSS_AM62L.PNG,width:30%}
\image html DMSS_AM62L.PNG "DMSS Block Diagram"
...
\imageStyle{TR_Types.png,width:30%}
\image html TR_Types.png "TR record fields"
...
Type 2   | 3D (word0-6)
Type 3   | 4D (word0-7)
Type 15  | 4D Block Copy with reformatting and indirection (word0-15)

### UDMA Setup/Flow

Below diagram shows the high level flow for the transfer requests from application and driver

\imageStyle{TRPD_flow_62L.png,width:50%}
\image html TRPD_flow_62L.png "UDMA setup TRPD flow"

Below diagram shows the UDMA transfer API flow

\imageStyle{UDMA_62L_API_flow.png,width:50%}
\image html UDMA_62L_API_flow.png "UDMA API flow"
