...
\image html am62px/snagfactory_flashing.png 
...
\image html am62px/snagfactory_cli.png 
...
- Edit the line below in the config file to point to your application `.appimage` file.
...
\note For HS-SE device, use DEVICE_TYPE=HS option in the makefile.
 ...
 - Ensure the am62px Processor SDK Linux path is correct in the `${SDK_INSTALL_PATH}/tools/boot/linuxAppimageGen/board/{board_name}/config.mak` file.
