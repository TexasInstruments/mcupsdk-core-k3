# Falcon Boot Support {#FALCON_BOOT}
...
\cond SOC_AM62X
- Refer to \htmllink{https://software-dl.ti.com/processor-sdk-linux/esd/AM62X/latest/exports/docs/linux/How_to_Guides_Developer_Notes.html, **Processor SDK Linux - Developer Notes**} guide to optimize the time taken by bootloader in the boot sequence.
\endcond
...
\attention Please modify the input/output filenames and path appropriately.
...
\cond SOC_AM62PX
- Refer to \htmllink{https://software-dl.ti.com/processor-sdk-linux/esd/AM62PX/latest/exports/docs/linux/Foundational_Components_ATF.html, **Processor SDK Linux - ARM Trusted Firmware-A**} to rebuild ATF inside the ATF source directory:
\endcond

\cond SOC_AM62X
- Refer to \htmllink{https://software-dl.ti.com/processor-sdk-linux/esd/AM62X/latest/exports/docs/linux/Foundational_Components_ATF.html, **Processor SDK Linux - ARM Trusted Firmware-A**} to rebuild ATF inside the ATF source directory:
\endcond

\cond SOC_AM62AX
- Refer to \htmllink{https://software-dl.ti.com/processor-sdk-linux/esd/AM62AX/latest/exports/docs/linux/Foundational_Components_ATF.html, **Processor SDK Linux - ARM Trusted Firmware-A**} to rebuild ATF inside the ATF source directory:
\endcond
...
\note SBL EMMC Linux boots from the boot partition 0 of eMMC and does not use a filesystem boot. The appimage is flashed to the offsets in eMMC as configured using syscfg.
...
\note The load address of resource table for R5 and M4 cores must be consistent with the address in the Linux dts file.
...
- \ref EXAMPLES_DRIVERS_SBL_EMMC_LINUX_MULTISTAGE for eMMC boot
- \ref EXAMPLES_DRIVERS_SBL_OSPI_LINUX_MULTISTAGE for OSPI boot
...
\note U-boot image is not required to be flashed as Falcon mode skips U-boot and jumps to kernel directly.
...
\note Falcon boot wih OSPI does not require config file or syscfg changes.
