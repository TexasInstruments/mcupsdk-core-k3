...
if __name__ == "__main__":
	...
	with open('threadx_doc_config.json', 'r') as json_file:
	...
	for device in j_dict["device"]:
		for os_name in j_dict["device"][str(device)]:
			...
			threadx_docs_enable_flag = 0
			for line in lines:
				#check if THREADX tag is present in includes.cfg
					if "ENABLED_SECTIONS" in line and "THREADX" in line:
						threadx_docs_enable_flag = 1

      	    #revise includes.cfg
			revise_cfg_file(includes_cfg_file_path, include_file_list, threadx_docs_enable_flag)

        	#revise examples.cfg
			examples_cfg_file_path = os.path.abspath("./device/"+str(device)+"/"+str(examples_cfg_file_name))
			revise_cfg_file(examples_cfg_file_path, example_md_list, threadx_docs_enable_flag)
			...
			for component_key in component_list.keys():
				...
				for root,dirs,files in os.walk(os.getcwd()):
					if component_cfg_file in files:
						...
						break
				revise_cfg_file(component_cfg_file_path, component_file_list_path, threadx_docs_enable_flag)
