...
::::{only} SOC_AM64X or SOC_AM243X or SOC_AM62X or SOC_AM62AX or SOC_AM62DX or SOC_AM62PX or SOC_AM275X or SOC_AM62LX
   ...
   - For low latency transfers, refer [EXAMPLES_DRIVERS_MCSPI_PERFORMANCE_32BIT](../../examples/drivers/mcspi_performance_32bit.md) and [EXAMPLES_DRIVERS_MCSPI_PERFORMANCE_8BIT](../../examples/drivers/mcspi_performance_8bit.md) example.
...
:::{only} SOC_AM64X or SOC_AM243X or SOC_AM62LX
...
:::{warning}
The use of NULL as a sentinel txBuf or rxBuf value to determine
...
::::{only} SOC_AM64X or SOC_AM243X
   ...
   - In case of DMA mode, as R5F core is not Cache Coherent, Cache Writeback is required if R5F writes to the buffers.
     And before reading the buffers, application needs to invalidate those. Please refer [MCSPI Loopback DMA](../../examples/drivers/mcspi_loopback_dma.md).
...
```{literalinclude} ../../../../docs/api_guide/doxy_samples/drivers/Mcspi_sample.c
:language: c
:lines: 4-4
```
...
```{doxygenfile} mcspi/v0/mcspi.h
```

```{doxygenfile} mcspi/v0/dma/mcspi_dma.h
```
