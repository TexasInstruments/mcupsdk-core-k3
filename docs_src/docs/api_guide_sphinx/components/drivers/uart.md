...
::::{only} SOC_AM64X or SOC_AM243X or SOC_AM62DX or SOC_AM62PX or SOC_AM275X or SOC_AM62LX
...
::::{only} SOC_AM64X or SOC_AM243X or SOC_AM62X or SOC_AM62AX or SOC_AM62DX or SOC_AM62PX or SOC_AM275X or SOC_AM62LX

   - For low latency direct register read/write API, refer [UART Echo Low Latency Interrupt](../../examples/drivers/uart_echo_low_latency_interrupt.md) and [UART Echo Low Latency Polling](../../examples/drivers/uart_echo_low_latency_polling.md) example.
...
::::{only} SOC_AM64X or SOC_AM243X or SOC_AM62DX or SOC_AM62PX or SOC_AM275X
...
::::{only} SOC_AM64X or SOC_AM243X
   ...
   - In case of DMA mode, as R5F core is not Cache Coherent, Cache Writeback is required if R5F writes to the buffers.
     And before reading the buffers, application needs to invalidate those. Please refer [UART Echo DMA](../../examples/drivers/uart_echo_dma.md).
...
```{literalinclude} ../../../../docs/api_guide/doxy_samples/drivers/Uart_sample.c
:language: c
:lines: 4-4
```
...
```{doxygenfile} uart/v0/uart.h
```

```{doxygenfile} uart/v0/dma/uart_dma.h
```
