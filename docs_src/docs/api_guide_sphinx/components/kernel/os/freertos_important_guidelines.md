...
<tr>
    <td> When <code>configUSE_TIME_SLICING</code> is 1, FreeRTOS will time slice between tasks at the same priority
    <td> Keep this at 0 (<b>SDK default config</b>), if this is not desired
</tr>
<tr>
    <td> Timer callbacks are called in task context vs ISR context in SysBIOS
    <td> Keep the timer task to highest priority so that timer callback gets called immediately after all ISRs are done (<b>SDK default config</b>)
</tr>
<tr>
    <td> No SWI support in FreeRTOS
    <td> Use “pend" function call, where functions are executed in timer task context. Again keep timer task as highest priority (<b>SDK default config</b>), see <code>xTimerPendFunctionCall()</code> and <code>xTimerPendFunctionCallFromISR()</code>
</tr>
<tr>
    <td> No ability to create arbitrary heaps
    <td> SDK DPL provides APIs to create arbitrary heaps to match SysBIOS (see <a href="../dpl/HeapP.html">Heap</a>)
</tr>
<tr>
    <td> Only FreeRTOS APIs ending with <code>FromISR()</code> can be called from within ISRs
    <td> SDK DPL provides a function <code>HwiP_inISR()</code> to detect if code is in ISR. This can be used to keep the application functions generic. SDK DPL takes care of this by default
</tr>
<tr>
    <td> Number of task priorities is configurable (upto max 32) vs SysBIOS default of 16
    <td> <b>SDK default config</b> keeps number of task priorities to 16 in FreeRTOS
</tr>
</table>
...
:::{only} SOC_AM62X or SOC_AM62DX or SOC_AM62LX
     - Stack size in units of “stack words”, i.e 64b or 8 bytes for A53
:::
...
:::{only} SOC_AM62LX
     - SDK default config is shown below, `vApplicationIdleHook` calls `wfi` instruction for A53.
:::
...
- In {{ VAR_SDK_NAME }} porting layer, we implement `portGET_RUN_TIME_COUNTER_VALUE()` using `ClockP_getTimeUsec` to return time stamp in units of usecs
...
- To get task load with overflow condition taken care of, it is recommended to create the task using [Task](../dpl/TaskP.md) DPL APIs
...
- ISR handler may or may not save FPU state depending on the CPU, see [Features Supported](freertos.md) to check if your CPU of interest supports FPU save/restore
...
:::{only} SOC_AM62X or SOC_AM62LX or SOC_AM62DX
     - For A53, we use one of the SOC level general purpose DM timers.
:::
...
:::{only} SOC_AM62AX or SOC_AM62X or SOC_AM62DX
...
- Therefore, SDK DPL implements heap APIs to match SysBIOS as below
  ...
  - `HeapP.h` is the user API ([Heap](../dpl/HeapP.md))
