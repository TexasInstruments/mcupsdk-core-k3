...
For feature/device coverage, memory footprint, and interoperability data, see [Time-Sensitive Networking (TSN)](enet_cpsw_tsn_landing_page.md). For step-by-step integration into an application, see [Ethernet TSN and gPTP Stack - API and Integration Guide](enet_cpsw_tsn_gptp_apiguide.md).
...
![](../../images/networking/gptp_roles.png)
...
![gPTP synchronization mechanism](../../images/networking/gptp_hw.png)
...
![Clock servo: shared Sync/Follow_Up measurements feed two independent loops](../../images/networking/clock_servo_architecture.png)
...
![Frequency loop stability state machine](../../images/networking/freq_stability_state_machine.png)
...
![Phase loop offset state machine](../../images/networking/phase_offset_state_machine.png)
...
![Default vs Quicksync Algorithm timelines](../../images/networking/quick_sync_timeline.png)
...
![TSN software stack: gPTP and AVTP both run on uniconf, combase, and unibase](../../images/networking/tsn_helper_libs_stack.png)
...
![Application write reaches CPSW hardware through uniconf's HAL and combase_link](../../images/networking/uniconf_hw_config_flow.png)
...
`ifname` is the same `combase` device name used everywhere else in TSN init (e.g. `"tilld0"`). See the full, working version — including the wait for gPTP sync before arming the schedule — in `est_init.c` under [Examples and Feature Coverage](enet_cpsw_tsn_landing_page.md#examples-and-feature-coverage) (listed on devices where the EST example is supported).
...
- [Time-Sensitive Networking (TSN)](enet_cpsw_tsn_landing_page.md) — TSN feature/standard coverage, resource utilization, memory footprint
...
- [TSN Developer Guidelines](enet_cpsw_tsn_developer_guide.md) — TSN developer guidelines (sync tuning, troubleshooting, limitations)
