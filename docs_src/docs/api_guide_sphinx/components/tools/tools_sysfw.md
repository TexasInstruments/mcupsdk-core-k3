...
::::{only} SOC_AM64X or SOC_AM243X or SOC_AM275X or SOC_AM62LX or SOC_AM263X or SOC_AM273X or SOC_AWR294X or SOC_J722S
...
::::{only} SOC_AM64X or SOC_AM243X or SOC_AM263X or SOC_AM273X or SOC_AWR294X
   - If you're not using any of the SBLs (SBL UART, SBL OSPI, SBL NULL) and is following the CCS boot method (`EVM_SOC_INIT_NOBOOT_MODE`), make sure to build the sciclient_set_boardcfg application by doing
...
```{rubric} TIFS and DM Trace Enable
```
...
::::{only} SOC_AM62X or SOC_AM62AX or SOC_AM62PX or SOC_AM62DX or SOC_AM275X
   ...
   **Default Configuration:**
   ...
   To enable **all** TIFS and DM traces, change the `#undef SYSFW_TRACE_ENABLE` to `#define SYSFW_TRACE_ENABLE` in `source/drivers/device_manager/sciclient.h`. Then rebuild the boardcfg as explained in the [SYSFW Board Config Generation](tools_sysfw.md) section. This enables **all** the following trace sources and destinations -
...
::::{only} SOC_AM62X or SOC_AM62AX or SOC_AM62PX or SOC_AM62DX or SOC_AM275X
   ...
   3. **Rebuild and reflash board configuration**:
      - Follow the instructions in [SYSFW Board Config Generation](tools_sysfw.md) section to regenerate boardcfg
   ...
   **Memory Destination** (`TISCI_BOARDCFG_TRACE_DST_MEM`):
   ...
   - Memory buffer location: See [TIFS memory buffer documentation](http://downloads.ti.com/tisci/esd/latest/4_trace/trace.html#trace-memory-buffer-location)
...
```
- For more details, refer [system firmware trace layer documentation](https://downloads.ti.com/tisci/esd/latest/4_trace/trace.html)
...
```
   ...
   - For more details on the TISCI message and the argument values to be used, refer [system firmware documentation](https://downloads.ti.com/tisci/esd/latest/6_topic_user_guides/secure_debug.html)
