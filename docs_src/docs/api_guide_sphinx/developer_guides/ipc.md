...
:::{only} SOC_AM62DX or SOC_AM62LX or SOC_AM275X
   :::{admonition} Note
   A53 and Linux will not be available on all SOCs. All references to Linux and A53 should be ignored on such SOCs.
   :::
:::
...
- Debug logging in multi-core environment
  - [Debug](../components/kernel/dpl/DebugP.md)
- Examples using IPC
  - [IPC Notify Echo](../examples/drivers/ipc_notify_echo.md)
  - [IPC RP Message Echo](../examples/drivers/ipc_rpmessage_echo.md)

:::{only} SOC_AM64X or SOC_AM243X
   - Mutual exclusion across CPUs
     - `DRIVERS_SPINLOCK_PAGE`
   - `EXAMPLES_DRIVERS_IPC_SPINLOCK_SHAREDMEM`
   - [IPC RP Message Linux Echo](../examples/drivers/ipc_rpmessage_linux_echo.md)
   - `EXAMPLE_MOTORCONTROL_BENCHMARKDEMO`
:::
...
:::{only} SOC_AM64X or SOC_AM243X
   ...
   ```{figure} ../images/ipc/ipc_block_diagram.png
   :align: center
...
:::{only} SOC_AM263X
   ...
   ```{figure} ../images/ipc/ipc_block_diagram_am263.png
   :align: center
...
:::{only} SOC_AM273X or SOC_AWR294X
   ...
   ```{figure} ../images/ipc/ipc_block_diagram_am273.png
   :align: center
...
:::{only} SOC_AM62X
   ...
   ```{figure} ../images/ipc/ipc_block_diagram_am62.png
   :align: center
...
:::{only} SOC_AM62AX
   ...
   ```{figure} ../images/ipc/ipc_block_diagram_am62a.png
   :align: center
...
:::{only} SOC_AM62DX
   ...
   ```{figure} ../images/ipc/ipc_block_diagram_am62d.png
   :align: center
...
:::{only} SOC_AM275X
   ...
   ```{figure} ../images/ipc/ipc_block_diagram_am275.png
   :align: center
...
:::{only} SOC_AM62PX
   ...
   ```{figure} ../images/ipc/ipc_block_diagram_am62p.png
   :align: center
...
- IPC Notify
  ...
  - Here, a user can send a 28b message ID along with the interrupt and there can be upto `IPC_NOTIFY_CLIENT_ID_MAX` logical end points or ports (called client ID's) associated with a CPU.
...
:::{only} SOC_AM64X or SOC_AM62X or SOC_AM62AX or SOC_AM62PX or SOC_AM62DX
   ### When using Linux
...
- The RP Message number of buffers (configured as `vringNumBuf` in SysConfig) **must be a power of 2**. The vring ring index is computed as `lastAvailIdx % numBuf`, where `lastAvailIdx` is a `uint16_t` that wraps from 65535 back to 0. If `numBuf` is not a power of 2, the modulo produces an incorrect slot index after the wrap, causing silent message corruption or stall. SysConfig enforces this constraint and will report an error if a non-power-of-2 value is used.
...
- A server CPU typically offers some service, say do some computation or read some sensor,
  ...
  - An end point is any 16b number, however in our implementation, we constrain it to `RPMESSAGE_MAX_LOCAL_ENDPT`, to make the implementation fit a
...
And the end point values MUST be less than `IPC_NOTIFY_CLIENT_ID_MAX`
...
:::{only} SOC_AM62DX or SOC_AM62LX or SOC_AM275X or SOC_AM62PX
   ...
   We use [IPC RP Message Echo](../examples/drivers/ipc_rpmessage_echo.md) example as reference to go through
:::
...
:::{only} SOC_AM62X or SOC_AM62AX or SOC_AM62PX
   ...
   We use [IPC RP Message Linux Echo](../examples/drivers/ipc_rpmessage_linux_echo.md) example as reference to go through
...
:::{only} SOC_AM64X or SOC_AM243X
   ...
   ```{figure} ../images/ipc/ipc_enable.png
   :align: center
...
:::{only} SOC_AM263X
   ...
   ```{figure} ../images/ipc/ipc_enable_am263.png
   :align: center
...
:::{only} SOC_AM273X or SOC_AWR294X
   ...
   ```{figure} ../images/ipc/ipc_enable_am273.png
   :align: center
...
:::{only} SOC_AM62X or SOC_AM62AX or SOC_AM62PX or SOC_AM62DX or SOC_AM275X
   ...
   ```{figure} ../images/ipc/ipc_enable_am62.png
   :align: center
   ...
   **IPC SysConfig**
   ...
   :::{only} SOC_AM62X or SOC_AM62AX or SOC_AM62PX or SOC_AM62DX
      - As only the IPC between A53 running Linux and MCU R5F is supported now, `Linux A53 IPC RP Message` is enabled by default after adding the IPC. This can not be disabled.
   :::
...
- This step needs to be repeated for every RTOS/NORTOS CPU. Using **system projects** in this case helps to view all the CPUs together in
  SysConfig. See [Using SDK with CCS Projects](ccs_projects.md) for using system projects with CCS and [Using SDK with Makefiles](makefile_build.md) for system projects with  makefile.
...
- When operating in RTOS/NORTOS ONLY mode, i.e no Linux, it is recommended to also enable debug logs via `Shared Memory Log Writer` on all CPUs and
  ...
  - See `Debug Log` under `TI DRIVER PORTING LAYER` in left tab in SysConfig to see how this is enabled in the IPC examples. See also [Debug](../components/kernel/dpl/DebugP.md) for more details.

### Update linker command file


:::{only} SOC_AM62PX or SOC_AM62LX or SOC_AM275X or SOC_AM62DX
   ...
   - This is done via the linker command file as shown in below snippet taken from  [IPC RP Message Echo](../examples/drivers/ipc_rpmessage_echo.md) example
   ...
   :::{only} SOC_AM64X
      ...
      - Refer to linker command file for [IPC RP Message Linux Echo](../examples/drivers/ipc_rpmessage_linux_echo.md) example for reference.
   :::
...
- Once again
  - Refer to MPU settings for each CPU in [IPC RP Message Echo](../examples/drivers/ipc_rpmessage_echo.md) example for RTOS/NORTOS applications WITHOUT Linux.

:::{only} SOC_AM64X or SOC_AM62X or SOC_AM62AX or SOC_AM62PX
- And refer to MPU settings for each CPU in [IPC RP Message Linux Echo](../examples/drivers/ipc_rpmessage_linux_echo.md) example when Linux is also present in the system
:::
...
- Now you can start sending messages between the enabled CPUs using the APIs defined in [IPC Notify](../components/drivers/ipc_notify.md) and
  [IPC RPMessage](../components/drivers/ipc_rpmsg.md)
