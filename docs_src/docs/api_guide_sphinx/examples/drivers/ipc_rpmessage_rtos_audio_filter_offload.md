...
- The DSP application on receiving the message from Master Core would trigger the audio filter signal chain on the buffer with the parameters and sends an acknowledgement to Master Core with the CPU task load.
...
Figure below shows the signal chain, where, the input is eight-channel, 256 block size audio data in channel interleaved form. The output is the processed eight-channel, 256 block size audio data in channel interleaved form.

The incoming audio stream is assumed to be sampled at 48KHz.

::::{only} SOC_AM62DX
![TISP audio signal chain](../../images/am62dx/ipc_rpmsg_linux_audio_filter_signalchain.png)
::::

::::{only} SOC_AM275X
![TISP audio signal chain](../../images/am275x/ipc_rpmsg_linux_audio_filter_signalchain.png)
::::
...
`${SDK_INSTALL_PATH}/source/tisp/test/TISP_idat_gen/audio_signal_chain/cascadeBiquad_FIR/` folder.

`cascadeBiquad0CoeffCase5` array in `${SDK_INSTALL_PATH}/examples/drivers/ipc/ipc_rpmsg_rtos_audio_filter_offload/{{ VAR_BOARD_NAME_LOWER }}/c75ss0-0_freertos/ti-c7000/TISP_cascadeBiquad_FIR_test.cpp` handles the coefficients that determines the cut-off and type of this filter.
...
`cascadeBiquad1CoeffCase5` array in `${SDK_INSTALL_PATH}/examples/drivers/ipc/ipc_rpmsg_rtos_audio_filter_offload/{{ VAR_BOARD_NAME_LOWER }}/c75ss0-0_freertos/ti-c7000/TISP_cascadeBiquad_FIR_test.cpp` handles the coefficients that determines the cut-off and type of this filter.
...
`filterCoeffCase5` array in `${SDK_INSTALL_PATH}/examples/drivers/ipc/ipc_rpmsg_rtos_audio_filter_offload/{{ VAR_BOARD_NAME_LOWER }}/c75ss0-0_freertos/ti-c7000/TISP_cascadeBiquad_FIR_test.cpp` handles the coefficients that determines the cut-off and type of this filter.
...
- To check the router connection with host PC, it is recommended to disconnect all other networking connections
...
Refer: [https://github.com/TexasInstruments/rpmsg-dma/blob/main/example/audio_offload/host%20utility/README.md](https://github.com/TexasInstruments/rpmsg-dma/blob/main/example/audio_offload/host%20utility/README.md)
