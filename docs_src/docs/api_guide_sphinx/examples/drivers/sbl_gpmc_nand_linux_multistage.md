# SBL GPMC NAND Linux Multistage
...
```{attention}
Board modification are required for supporting GPMC NAND flash device daughter card. Refer the board user guide of SK-AM62-LP for details on the modifications.
```
...
- Refer [HSM_APPIMAGE_GEN_TOOL](#hsm_appimage_gen_tool) for more details
...
```{note}
For HS-SE device, use **default_sbl_gpmc_linux_hs.cfg** as the cfg file.
```
```{note}
For HS-FS device, use **default_sbl_gpmc_linux_hs_fs.cfg** as the cfg file.
```

```{attention}
The UART uniflash does not have the GPMC flash enabled by default. Enable GPMC by adding another flash instance with 'Flash Topology' as 'Parallel Flash'. It will add GPMC module as well.
```
...
::::{only} SOC_AM62X
```{note}
The above config file will flash till u-boot on the GPMC NAND. U-Boot can load kernel from any bootmedia, refer to \htmllink{https://software-dl.ti.com/processor-sdk-linux/esd/AM62X/latest/exports/docs/devices/AM62X/linux/Overview.html, **Processor SDK Linux**} user guide for more details.
::::
```
