...
```{attention}
While using device variants, please remove the cores that are not present on the device from the **BOOTLOADER** section of the example Syscfg
```
...
- Refer [HSM_MCELF_IMAGE_GEN_TOOL](#hsm_mcelf_image_gen_tool) for more details
::::
...
```{note}
For HS-SE device, use **sbl_ospi_hs.cfg** as the cfg file.
```
```{note}
For HS-FS device, use **sbl_ospi_hs_fs.cfg** as the cfg file.
```
...
- Example, assuming SDK is installed at `C:/ti/mcu_plus_sdk` and this example and IPC application is built using makefiles, and RTOS Appimage is already created, in Windows,
        ...
        python uart_uniflash.py -p COM13 --cfg=C:/ti/mcu_plus_sdk/tools/boot/sbl_prebuilt/{{ VAR_BOARD_NAME_LOWER }}/sbl_ospi_hs_fs.cfg

- If Linux PC is used, assuming SDK is installed at `~/ti/mcu_plus_sdk`

        cd ~/ti/mcu_plus_sdk/tools/boot
        python uart_uniflash.py -p /dev/ttyUSB0 --cfg=sbl_prebuilt/{{ VAR_BOARD_NAME_LOWER }}/sbl_ospi_hs_fs.cfg
...
c75ss0
c75ss1
[KPI] System_init                      :      36409us
[KPI] Drivers_open                     :        144us
[KPI] Board_driversOpen                :      27029us
[KPI] App_loadImages                   :      12539us
[KPI] SBL Total Time Taken             :      76123us
