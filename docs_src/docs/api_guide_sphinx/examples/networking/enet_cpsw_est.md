...
![](../../images/networking/cpsw_est_am62x.png)
...
The application uses ENET_MOD_TAS API for EST configuration and ENET_DMA_API
...
![](../../images/examples/CpswEstExample_DfltSchedule.png)
...
- ENET_TAS_IOCTL_SET_STATE to set the EST state to *RESET* (ENET_TAS_RESET)
  before any other EST/TAS IOCTL.  And subsequently, after the administrative
  list has been programmed, to set EST state to *ENABLE*  (ENET_TAS_ENABLE).
...
- CPSW_MACPORT_IOCTL_EST_ENABLE_TIMESTAMP to enable timestamping on a given
...
- CPSW_MACPORT_IOCTL_EST_DISABLE_TIMESTAMP to disable timestamping on a given
  MAC port.
- CPSW_CPTS_IOCTL_LOOKUP_EST_EVENT to retrieve timestamp events from the
...
![](../../images/examples/CpswEst_ExampleApp.png)
...
:::{note}
Ethernet cable must be connected and link must be up in order for the example
...
:::{note}
Timestamp check mechanism used to determined test status is enabled only with
