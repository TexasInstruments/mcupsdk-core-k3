...
Change the value of `priority1` in ~/gptp_config.cfg file to `255` to enforce PC to gPTP slave
:::
...
::::{only} SOC_AM62PX
   ...
   - Connect a ethernet cable to the CPSW RGMII 1 port in AM62PX-SK from host PC as shown below
...
EVM and PC has to be connected directly as shown below using CAT6 or CAT5 cable. If there is ethernet switch placed in between, make sure the switch is gPTP capable.
  ...
  ![Local network between PC and EVM](../../images/networking/gptp_topology_evm_pc.png)
