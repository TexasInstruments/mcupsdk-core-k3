...
:::{admonition} Note
Set this macro **UPDATE_KEYREV** in the `{SDK_PATH}/examples/otp/runtime_keyrev/{BOARD}/r5fss0-0_nortos/main.c` to update the KEY Revision.
::::
...
- Change the working directory to `{SDK_PATH}/tools/boot/signing`.
...
- Convert the `dual_cert_keyrev.bin` into a header file `dual_cert_keyrev.h` using the following command.
      ...
      python3 bin2c.py ../boot/signing/dual_cert_keyrev.bin dual_cert_keyrev.h KEYREV_CHANGE_CERT 2025
...
::::{only} SOC_AM62AX
   ...
   - After successful flashing, power off the board and set the bootmode to OSPI NAND [OSPI SERIAL NAND BOOT MODE](../../getting_started/am62px_evm_setup.md).
   - Power on the board. The example should boot with the logs appearing on WKUP_UART0.
...
::::{only} SOC_AM62PX
         ...
         python uart_uniflash.py -p {COM_PORT} --cfg=../../examples/otp/runtime_keyrev/{{ VAR_BOARD_NAME_LOWER }}/wkup-r5fss0-0_nortos/default_runtime_keyrev_hs.cfg

   - After successful flashing, power off the board and set the bootmode to OSPI [OSPI NOR BOOT MODE](../../getting_started/am62px_evm_setup.md).
   - Power on the board. The example should boot with the logs appearing on WKUP_UART0.
...
::::{only} SOC_AM62X
         ...
         python uart_uniflash.py -p {COM_PORT} --cfg=../../examples/otp/runtime_keyrev/{BOARD}/r5fss0-0_nortos/default_runtime_keyrev_hs.cfg
