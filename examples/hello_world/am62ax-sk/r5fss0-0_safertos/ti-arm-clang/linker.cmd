 ...
 *   uses this stack.
 * - After vTaskStartScheduler() each task created in SafeRTOS has its own stack
 */
--stack_size=0x8000
/* This is the heap size for malloc() API in NORTOS and SafeRTOS
 * This is also the heap used by pvPortMalloc in SafeRTOS
 */
--heap_size=0x10000
 ...
 * - Hence in SafeRTOS, IRQ stack size is less and SVC stack size is more
 */
__IRQ_STACK_SIZE = 0x2000;
/* This is the size of stack when R5 is in IRQ mode
 * - In both NORTOS and SafeRTOS nesting is disabled for FIQ
 */
__FIQ_STACK_SIZE = 0x01000;
__SVC_STACK_SIZE = 0x01000; /* This is the size of stack when R5 is in SVC mode */
__ABORT_STACK_SIZE = 0x01000;  /* This is the size of stack when R5 is in ABORT mode */
__UNDEFINED_STACK_SIZE = 0x01000;  /* This is the size of stack when R5 is in UNDEF mode */
...
SECTIONS
{
    ...
    .KERNEL_DATA palign( 0x0400 ),
                LOAD_START( lnkKernelDataStartAddr ),
                LOAD_END( lnkKernelDataEndAddr )                : {} > DDR
...
MEMORY
{
    ...
    DDR_LPM_DATA                 : ORIGIN = 0x9CA00000 LENGTH = 0x000A0000
    /* DDR for DM RM/PM HAL trace buffer [ size 20 KB ] */
    DDR_DM_RMPM_TRACE            : ORIGIN = 0x9CAA0000 LENGTH = 0x00005000
    /* DDR for DM R5F code/data [ size 27 MiB + 364 KB ] */
    DDR                          : ORIGIN = 0x9CAA5000 LENGTH = 0x1B5B000   /* Code/Data */
