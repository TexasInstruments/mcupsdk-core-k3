    ...
    if(config_file != None):
        ...
        if(parse_status != 0):
            ...
            if(filecfg.bootloader_index != None):
                ...
                # AM275x: ROM loads the SBL directly from the .tiimage — no EOFT needed.
                # Just wait for the SBL to boot and start its XMODEM receive loop.
                if(device == "am275x"):
                    print("Waiting for SBL to initialize ...")
                    time.sleep(5)
                    print("")
