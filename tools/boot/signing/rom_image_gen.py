...
bootCore = INTEGER:16
compOpts = INTEGER:0
destAddr = FORMAT:HEX,OCT:{BOARDCFG_SBLDATA_LOAD_ADDR}
compSize = INTEGER:{BOARDCFG_SBLDATA_SIZE}
shaType = OID:{SHA_OID}
shaValue = FORMAT:HEX,OCT:{BOARDCFG_SBLDATA_SHA_VAL}
'''
...
def get_cert(args):
    ...
    if args.enable_sbldata == 'yes':
        full_image_size += os.path.getsize(args.boardcfg_sbldata_blob)
        sbldata_c_ext = g_sbldata_cert_ext
        sbldata_c_seq = g_sbldata_cert_seq.format(
