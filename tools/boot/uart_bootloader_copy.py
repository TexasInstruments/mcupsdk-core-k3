        ...
        if(parse_status != 0):
            ...
            for i in range(0, len(filecfg.cfgs)):
                if((i != filecfg.bootloader_index) and (i != filecfg.soc_index)):
                    ...
                    # After sending the first file (DM appimage / secondary bootloader),
                    # send EOFT to the first-stage SBL so it runs the DM R5 core.
                    # Then wait for the secondary bootloader to boot and be ready for XMODEM.
                    if not first_file_sent:
                        first_file_sent = True
                        print("Sending End Of File Transfer to first-stage SBL ...")
                        eoft_filename = "eoft_dm.dat"
                        eoft_file     = open(eoft_filename, "wb")
                        eoft_bytes    = BOOTLOADER_END_OF_FILES_TRANSFER.to_bytes(BOOTLOADER_END_OF_FILES_TRANSFER_WORD_LENGTH, "big")
                        eoft_file.write(eoft_bytes)
                        eoft_file.close()
                        eoft_status, eoft_timetaken = xmodem_send_receive_file(eoft_filename, serialport, get_response=False)
                        os.remove(eoft_filename)
                        print("Sent EOFT to first-stage SBL in {}s.".format(eoft_timetaken))
                        print("Waiting for secondary bootloader to initialize ...")
                        time.sleep(5)
                        print("")
