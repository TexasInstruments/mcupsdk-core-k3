...
def create_blob(args):
    ...
    # Assume 0 board configs initially present and then calculate based on the args
    num_elems = 0

    if(args.bcfg is not None):
        num_elems = num_elems + 1

    if(args.bcfg_rm is not None):
        num_elems = num_elems + 1

    if(args.bcfg_pm is not None):
        num_elems = num_elems + 1

    if(args.bcfg_sec is not None):
        num_elems = num_elems + 1
